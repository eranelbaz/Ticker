import { aggregateLiveBar } from './live-aggregator';
...
describe('aggregateLiveBar', () => {
  ...
  it('merges a bar within the same bucket, keeping time and open', () => {
    ...
    expect(aggregateLiveBar(lastDaily, bar, dayTf)).toEqual(
      c(1000, 10, 15, 8, 9, 150),
    );
  });

  it('keeps merging successive bars in the same bucket', () => {
    const first = aggregateLiveBar(lastDaily, c(1060, 11, 13, 10, 12, 50), dayTf);
    const second = aggregateLiveBar(first, c(1120, 12, 16, 7, 14, 25), dayTf);
  ...
  it('skips empty buckets but stays on the grid (multiple tf ahead)', () => {
    ...
    expect(aggregateLiveBar(lastDaily, bar, dayTf).time).toBe(1000 + 3 * dayTf);
  });

  it('works for a 1-minute chart too (tf=60)', () => {
    const last = c(600, 5, 6, 4, 5.5, 10);
    expect(aggregateLiveBar(last, c(630, 5.5, 7, 5, 6, 4), 60)).toEqual(
      c(600, 5, 7, 4, 6, 14),
    );
    expect(aggregateLiveBar(last, c(660, 6, 6.5, 5.8, 6.2, 3), 60)).toEqual(
  ...
  it('never moves time backwards when the live bar is earlier in the same bucket', () => {
    ...
    const folded = aggregateLiveBar(last, earlierLiveBar, dayTf);
